...
static unsigned char udp_body[UDP_BODY_LEN] = {0};
...
print_help(void)
{
  printf("\n\npfudpbench(C)    2013 Zhu Youzhi<zhuyouzhi@gmail.com>\n\n");
  printf("Usage: pfudpbench -i device [-b bodydata] [-r rate] [-c count]");
  printf(" --dmac nexthopmac --dip dstip [--dport dstport] [--sip srcip]\n\n");
  printf("-i <device>        Device name [Man]\n");
  printf("-b <bodydata>      Udp body [Opt, NULL as default]\n");
...
packet_sender_thread(void* id)
{
  ...
  int udp_body_len = 0;
  int udp_csum_len = DEFAULT_UDP_HDR_LEN;
  ...
  if (strlen((char *)udp_body) > 0){
    udp_body_len = strlen((char *)udp_body);
...
main(int argc, char* argv[])
{
  ...
  while ((opt = getopt_long(argc, argvopt, "hi:r:c:b:",
    ...
    case 'b':
      {
        int len = ((UDP_BODY_LEN - 1) <= (strlen(optarg)))?(UDP_BODY_LEN - 1):strlen(optarg);
        strncpy((char *)udp_body, optarg, len);
      }
      break;
    case 0:
  ...
  for(i=0; i<num_channels; i++) {
    ...
    snprintf(buf, sizeof(buf), "pfudpbench-thread %ld", i);
