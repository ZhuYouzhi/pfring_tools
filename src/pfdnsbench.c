...
#define DEFAULT_IP_TOT_LEN     0x2800
#define IP_STR_LEN             16

#define MAX_PORT_NUM           65536
#define UDP_BODY_LEN           1024
#define DEFAULT_UDP_HDR_LEN    8
...
static u_int32_t expected_pps = 9000000;
static u_int64_t expected_pkts_num = 0;
static char str_src_ip[IP_STR_LEN] = {"192.168.1.1"};
static char str_dst_ip[IP_STR_LEN] = {"192.168.1.2"};
static u_int16_t dport = DEFAULT_DNS_PORT;
 ...
 * |              .....              |
 */
static unsigned char udp_body[UDP_BODY_LEN] = {
0xD5, 0xB9, /* ID */
0x01, 0x00, /* Flag */
0x00, 0x00, /* Question number, refilled after parse */
0x00, 0x00, /* Answer RRs */
0x00, 0x00, /* Authority RRs */
0x00, 0x00 /* Additional RRs */
};

static int domain_name_num = 0;
static unsigned char domain_names[16][64];
static int dns_body_len = 0;

static unsigned char pseudo_udp_data[UDP_BODY_LEN] = {0};
...
/*
 * Dump udp pseudo header fields
 */
void
dump_udp_pseudo_hdr(udp_pseudo_hdr_t *hdr)
{
  printf("sizeof: %lu\n", sizeof(udp_pseudo_hdr_t));
  printf("sip: %u dip: %u holder: %d proto: %d length: %d ",
    hdr->srcip, hdr->dstip, hdr->placeholder, hdr->protocol, ntohs(hdr->udp_length));
  printf("sport: %u dport: %u length: %u check: %u\n",
    hdr->udp.source, hdr->udp.dest, ntohs(hdr->udp.len), hdr->udp.check);
}
...
print_help(void)
{
  printf("\n\npfdnsbench(C)    2013 Zhu Youzhi<zhuyouzhi@gmail.com>\n\n");
  printf("Usage: pfdnsbench -i device [-d dnsnames] [-r rate] [-c count]");
  printf(" --dmac nexthopmac --dip dstip [--dport dstport] [--sip srcip]\n\n");
  printf("-i <device>        Device name [Man]\n");
  printf("-d <domain names>  DNS domain names seperated by space or tab [Man]\n");
  ...
  printf("--dport <dstport>  Specify destination port number [Opt, 53 as default]\n");
...
packet_sender_thread(void* id)
{
  ...
  int udp_pkt_len = ETH_HLEN + DEFAULT_IP_HDR_LEN + DEFAULT_UDP_HDR_LEN;
  int udp_body_len = dns_body_len; /* length of dns body */
  int udp_csum_len = DEFAULT_UDP_HDR_LEN; /* length to calc udp check data length */
  ...
  udp_body_len = dns_body_len;
  udp_csum_len += udp_body_len;
  udp_pkt_len += udp_body_len;
  ...
  iph->tot_len = htons(DEFAULT_IP_HDR_LEN + udp_body_len + DEFAULT_UDP_HDR_LEN);
  ...
  /*
   * Fill udp packet members
   */
  struct udphdr *udph = (struct udphdr *) (buffer + DEFAULT_IP_HDR_LEN + ETH_HLEN);
  udph->dest = htons(dport);
  udph->len = htons(udp_body_len + DEFAULT_UDP_HDR_LEN);

  /*
   * Fill pseudo udp header members, for checksum computation
   */
  int udp_psd_hdr_len = sizeof(udp_pseudo_hdr_t);
  udp_pseudo_hdr_t *psh = (udp_pseudo_hdr_t *)pseudo_udp_data;
  psh->dstip = inet_addr(str_dst_ip);
  psh->placeholder = 0;
  psh->protocol = IPPROTO_UDP;
  psh->udp_length = htons(udp_body_len + DEFAULT_UDP_HDR_LEN);
  if (udp_body_len > 0) {
    memcpy(buffer + ETH_HLEN + DEFAULT_IP_HDR_LEN + DEFAULT_UDP_HDR_LEN,
        udp_body, udp_body_len);
    memcpy(pseudo_udp_data + udp_psd_hdr_len, udp_body, udp_body_len);
  }
  ...
  udph->source = rand_val % MAX_PORT_NUM;
  memcpy(&(psh->udp) , udph , DEFAULT_UDP_HDR_LEN);
  ...
  while(1){
    ...
    if (rate_control == 1){
      ...
      for(i = 0; i < expected_pps; i++){
        ...
        psh->srcip = sip;
        psh->udp.check = 0;
        udph->check = csum((unsigned short*)psh, udp_psd_hdr_len + udp_csum_len);

        pfring_send(pd, buffer, udp_pkt_len, 0 );
        pkts_sent[thread_id]++;
...
void generate_dns_question(char *str, int start_idx, int end_idx)
{
  if (str == NULL) {
    return;
  }

  char *ptr = str + start_idx;

  unsigned char *curbuf = domain_names[domain_name_num];
  int curbufidx = 0;
  ...
  for(i=0; i<len; i++){
    ...
    case 0x2E: /*'.'*/
      if (sidx != -1 && eidx == -1){
        eidx = i;
        if (eidx > sidx){
          unsigned char clen = eidx-sidx;
          memcpy(curbuf + curbufidx, &clen, 1);
          curbufidx++;
          memcpy(curbuf + curbufidx, ptr+sidx, clen);
          curbufidx += clen;
          sidx = -1;
          eidx = -1;
        }
  ...
  if ((sidx != -1) && (sidx < (len - 1))){
    unsigned char clen = len-sidx;
    memcpy(curbuf + curbufidx, &clen, 1);
    curbufidx++;
    memcpy(curbuf + curbufidx, ptr+sidx, clen);
    curbufidx += clen;
...
/*
 * Generate dns body based on user input
 */
void generate_dns_body(char *str)
{
  if (str == NULL) {
    return;
  }
    ...
    case 0x09: /* Tab */
      if (start_idx != -1 && end_idx == -1){
        ...
        if (end_idx > start_idx){
          generate_dns_question(str, start_idx, end_idx);
          start_idx = -1;
          end_idx = -1;
        }
      }
  ...
  if ((start_idx != -1) && (start_idx < (len - 1))){
    generate_dns_question(str, start_idx, len);
  }

  /*
   * Make up dns request body
   */
  unsigned char question_end[] = {0x00, 0x00, 0x01, 0x00, 0x01};
  int q_size = sizeof(question_end);

  dns_body_len = DNS_QUERY_OFFSET;

  for(i = 0; i < domain_name_num; i++){
    memcpy(udp_body + dns_body_len, domain_names[i], strlen((char *)(domain_names[i])));
    dns_body_len += strlen((char *)(domain_names[i]));
    memcpy(udp_body+dns_body_len, question_end, q_size);
    dns_body_len += q_size;
  }

  /*
   * Fill actual question num
   */
  unsigned char clen = domain_name_num;
  memcpy(udp_body + q_size, &clen, 1);
}
...
main(int argc, char* argv[])
{
  ...
  while ((opt = getopt_long(argc, argvopt, "hi:r:c:d:",
    ...
    case 'd':
      {
        char buf[512];
        int len = ((UDP_BODY_LEN/2 - 1) <= (strlen(optarg)))?
                   (UDP_BODY_LEN/2 - 1):strlen(optarg);
        strncpy(buf, optarg, len);
        generate_dns_body(buf);
      }
      break;
    case 0:
  ...
  for(i=0; i<num_channels; i++) {
    ...
    snprintf(buf, sizeof(buf), "pfdnsbench-thread %ld", i);
