...
#define DEFAULT_TIME_PERIOD_US 1000
...
#define MAX_TCP_WINDOW         5840
#define DEFAULT_TCP_DOFF       5
#define DEFAULT_TCP_SEQ        123456
#define DEFAULT_TCP_HDR_LEN    20

#define DEFAULT_PKT_LEN        54
...
static u_int8_t th_flags = TH_SYN;
...
print_help(void)
{
  printf("\n\npfsynbench(C)    2013 Zhu Youzhi<zhuyouzhi@gmail.com>\n\n");
  printf("Usage: pfsynbench -i device [-f tcpflags] [-r rate] [-c count]");
  printf(" --dmac nexthopmac --dip dstip [--dport dstport] [--sip srcip]\n\n");
  printf("-i <device>        Device name [Man]\n");
  printf("-f <tcpflags>      TCP flags:SAFRPU, syn is set if not specified [Opt, S as default]\n");
...
packet_sender_thread(void* id)
{
  ...
  /*
   * Fill tcp packet members
   */
  struct tcphdr *tcph = (struct tcphdr *) (buffer + sizeof (struct ip) + ETH_HLEN);
  tcph->dest = htons(dport);
  tcph->seq = DEFAULT_TCP_SEQ;
  tcph->doff = DEFAULT_TCP_DOFF;
  if (th_flags & TH_FIN){
    tcph->fin=1;
  }
  if (th_flags & TH_SYN){
  ...
  tcph->window = htons (MAX_TCP_WINDOW);
  tcph->check = 0;
  tcph->urg_ptr = 0;

  /*
   * Fill pseudo tcp header members, for checksum computation
   */
  int tcp_psd_hdr_len = sizeof(tcp_pseudo_hdr_t);
  tcp_pseudo_hdr_t psh;
  psh.dstip = inet_addr(str_dst_ip);
  psh.placeholder = 0;
  psh.protocol = IPPROTO_TCP;
  psh.tcp_length = htons(DEFAULT_TCP_HDR_LEN);
  ...
  tcph->source = rand_val % MAX_PORT_NUM;
  memcpy(&psh.tcp , tcph , DEFAULT_TCP_HDR_LEN);
  ...
  while(1){
    ...
    if (rate_control == 1){
      ...
      for(i = 0; i < expected_pps; i++){
        ...
        psh.srcip = sip;
        psh.tcp.check = 0;
        tcph->check = csum((unsigned short*)&psh, tcp_psd_hdr_len);
...
main(int argc, char* argv[])
{
  ...
  while ((opt = getopt_long(argc, argvopt, "hi:r:c:f:",
  ...
  for(i=0; i<num_channels; i++) {
    ...
    snprintf(buf, sizeof(buf), "pfsynbench-thread %ld", i);
